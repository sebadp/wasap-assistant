...
async def test_get_current_datetime_utc():
    ...
    result = await reg.execute_tool(ToolCall(name="get_current_datetime", arguments={"timezone": "UTC"}))
...
async def test_get_current_datetime_invalid_tz():
    reg = _make_registry()
    from app.skills.models import ToolCall
    result = await reg.execute_tool(ToolCall(name="get_current_datetime", arguments={"timezone": "Fake/Place"}))
    assert result.success
