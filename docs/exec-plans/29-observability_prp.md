...
- [x] Extender `ChatResponse` en `app/llm/client.py`:
...
- [x] Agregar tests en `tests/test_llm_client.py`:
...
- [x] Refactorear `TraceRecorder.__init__()` en `app/tracing/recorder.py`:
...
- [x] Agregar `get_trace_recorder()` en `app/dependencies.py` (misma pattern que `get_repository()`)
- [x] Tests: `test_trace_recorder_singleton_reused` — verificar que la misma instancia se usa

## Phase 3: Span de generación LLM en flujo normal

- [x] En `app/skills/executor.py`:
...
- [x] En `app/webhook/router.py`:
  ...
  - Para el fallback `ollama_client.chat()` (sin tools), agregar span `kind="generation"`:
    ...
    ```
- [x] Tests en `tests/test_tool_executor.py`:
...
- [x] En `app/agent/loop.py`, dentro de `run_agent_session()`:
...
- [x] `tests/test_tracing.py` (nuevos o extender existentes):
...
- [x] `tests/test_agent_tracing.py` (nuevo archivo):
...
- [x] Correr `make check` (lint + typecheck + tests) — 481 tests pasan, 0 errores de linter, mypy errores pre-existentes en Langfuse stubs
...
- [x] Actualizar `docs/exec-plans/README.md` con entrada 29
- [x] Crear `docs/features/29-observability.md` con documentación de la feature
- [x] Crear `docs/testing/29-observability_testing.md` con guía de testing
