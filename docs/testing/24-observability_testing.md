...
El LocalForge imprimirá `Langfuse tracing enabled` durante la inicialización.
...
|---|---|
| Enviar un mensaje "Hola ¿quién sos?" a LocalForge | En `http://localhost:3000` aparecerá un trace llamado `interaction` en la solapa de Traces, con múltiples layers adentro de la cascada. |
...
|---|---|
| Hablarle a LocalForge mientras el servidor de Langfuse local está caído. | El bot igual responde sin tirarle error al usuario. En el terminal de Docker de `localforge` dirá "[WARNING] Failed to upload trace". |
...
docker compose logs -f localforge 2>&1 | grep -i "Intent Classifier"

# Ver payloads brutos que entran a las tools de ejecución (mcp)
docker compose logs -f localforge 2>&1 | grep -i "Tool Execution RAW"

# Ver la salida cruda literal parseada de audio
docker compose logs -f localforge 2>&1 | grep -i "Audio (Whisper) RAW INTERPRETATION"
```
...
sqlite3 data/localforge.db "SELECT * FROM traces ORDER BY start_time DESC LIMIT 1;"
sqlite3 data/localforge.db "SELECT COUNT(*) FROM trace_spans;"
```
...
|---|---|---|
| Las trazas no aparecen en el portal de Langfuse | Claves en `.env` no actualizadas. | Crea un proyecto nuevo en la UI de Langfuse, emití claves para API, pegalas en el archivo `.env` del bot y reiniciá `docker compose restart localforge`. |
